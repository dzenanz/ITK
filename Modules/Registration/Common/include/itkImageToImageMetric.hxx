...
ImageToImageMetric<TFixedImage, TMovingImage>::Initialize()
{
  ...
  if (this->m_UseFixedImageIndexes)
  {
    ...
    {
      itkExceptionStringMacro("FixedImageIndexes list is empty");
    }
  }
  else
  {
    ...
    {
      itkExceptionStringMacro("FixedImageRegion is empty");
    }

    if (!m_FixedImageRegion.Crop(m_FixedImage->GetBufferedRegion()))
    {
      itkExceptionStringMacro("FixedImageRegion does not overlap the fixed image buffered region");
    }
  }
