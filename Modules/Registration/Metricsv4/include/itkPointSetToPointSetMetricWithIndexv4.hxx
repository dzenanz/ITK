...
PointSetToPointSetMetricWithIndexv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::Initialize()
{
  ...
  if (this->HasLocalSupport())
  {
    if (!this->m_UserHasSetVirtualDomain)
    {
      ...
      {
        itkExceptionStringMacro("Expected the moving transform to be of type DisplacementFieldTransform or derived, or "
                                "a CompositeTransform with DisplacementFieldTransform as the last to have been added.");
      }
...
template <typename TFixedPointSet, typename TMovingPointSet, class TInternalComputationValueType>
typename PointSetToPointSetMetricWithIndexv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::
  MeasureType
