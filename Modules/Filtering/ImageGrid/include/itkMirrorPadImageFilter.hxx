                                                                                ...
                                                                                double &                outDecayFactor)
{
  ...
  for (unsigned int dimCtr = 0; dimCtr < ImageDimension; ++dimCtr)
  {
    ...
    const long a = outputRegionStart[dimCtr];
    const long c = inputRegionStart[dimCtr];

    if (oddRegionArray[dimCtr])
    {
      const long b = inputSizes[dimCtr];
  ...
  if (this->m_DecayBase != 1.0)
  {
    SizeValueType distanceFromEdge = 0;

    // city-block distance, first layer of 6-connected outside voxels having distance 1
    // 18-connected voxels and second 6-connected layer having distance 2 etc.
    for (unsigned int dimCtr = 0; dimCtr < ImageDimension; ++dimCtr)
    {
      distanceFromEdge += (itk::Math::Absolute(outputIndex[dimCtr] - inputIndex[dimCtr]) + 1) / 2;
    }
...
template <typename TInputImage, typename TOutputImage>
int
MirrorPadImageFilter<TInputImage, TOutputImage>::RegionIsOdd(long base, long test, long size)
{
  ...
  if (test < base)
  {
    const long oddness = (base - test - 1) / size;
    return !(oddness & 1);
  }
...
template <typename TInputImage, typename TOutputImage>
int
MirrorPadImageFilter<TInputImage, TOutputImage>::BuildInterRegions(std::vector<long> & inputRegionStart,
...
template <typename TInputImage, typename TOutputImage>
int
MirrorPadImageFilter<TInputImage, TOutputImage>::BuildPreRegions(std::vector<long> & inputRegionStart,
                                                                 std::vector<long> & outputRegionStart,
                                                                 std::vector<long> & inputRegionSizes,
                                                                 std::vector<long> & outputRegionSizes,
                                                                 long                inputIndex,
                                                                 ...
                                                                 int &               regCtr)
{
  ...
  // the input.
  // Holder for current size calculation.
  long sizeTemp = outputRegionStart[0] - outputIndex;
  sizeTemp = ((sizeTemp > 0) ? (sizeTemp % inputSize) : 0);
  outputRegionSizes[regCtr] = sizeTemp;
  inputRegionSizes[regCtr] = sizeTemp;
  long offset = inputSize - sizeTemp; // Offset for times when we need to shorten both ends.
  ...
  if ((outputRegionStart[regCtr] + static_cast<long>(outputRegionSizes[regCtr])) > (outputIndex + outputSize))
  {
    outputRegionSizes[regCtr] = outputIndex + outputSize - outputRegionStart[regCtr];
    inputRegionSizes[regCtr] = outputRegionSizes[regCtr];
    if ((inputRegionSizes[regCtr] < inputSize) && this->RegionIsOdd(inputIndex, outputRegionStart[regCtr], inputSize))
    {
...
template <typename TInputImage, typename TOutputImage>
int
MirrorPadImageFilter<TInputImage, TOutputImage>::BuildPostRegions(std::vector<long> & inputRegionStart,
                                                                  std::vector<long> & outputRegionStart,
                                                                  std::vector<long> & inputRegionSizes,
                                                                  std::vector<long> & outputRegionSizes,
                                                                  long                inputIndex,
                                                                  ...
                                                                  int &               regCtr)
{
  ...
  ++regCtr;
  long sizeTemp = outputIndex + outputSize - inputIndex - inputSize;
  sizeTemp = ((sizeTemp > 0) ? (sizeTemp % inputSize) : 0);
  outputRegionSizes[regCtr] = sizeTemp;
  inputRegionSizes[regCtr] = sizeTemp;
  outputRegionStart[regCtr] = outputIndex + outputSize - sizeTemp;
  int offset = inputSize - sizeTemp;
  ...
  for (int ctr = numRegs - 1; ctr >= 1; ctr--)
  {
    offset = 0;
    ++regCtr;
  ...
  if (outputRegionStart[regCtr] < outputIndex)
  {
    ...
    if (this->RegionIsOdd(inputIndex, outputRegionStart[regCtr], inputSize) && (outputIndex > (inputIndex + inputSize)))
    {
      inputRegionStart[regCtr] = inputIndex + offset;
    }
    else
...
template <typename TInputImage, typename TOutputImage>
void
MirrorPadImageFilter<TInputImage, TOutputImage>::GenerateInputRequestedRegion()
{
  ...
  for (unsigned int dimCtr = 0; dimCtr < ImageDimension; ++dimCtr)
  {
    ...
    numPre[dimCtr] =   // Count how many versions of input fit in pre-pad
      this->FindRegionsInArea(outputIndex[dimCtr],
                              inputIndex[dimCtr],
                              static_cast<long>(inputSize[dimCtr]),
                              inputIndex[dimCtr] - outputIndex[dimCtr] - static_cast<long>(outputSize[dimCtr]));
    numPost[dimCtr] = // Count how many versions of input fit in post-pad
      this->FindRegionsInArea(inputIndex[dimCtr] + static_cast<long>(inputSize[dimCtr]),
                              ...
                              static_cast<long>(inputSize[dimCtr]),
                              outputIndex[dimCtr] - inputIndex[dimCtr] - static_cast<long>(inputSize[dimCtr]));
  ...
  for (unsigned int dimCtr = 0; dimCtr < ImageDimension; ++dimCtr)
  {
    ...
    // side of this region.
    //
    int regCtr = BuildInterRegions(inputRegionStart[dimCtr],
  ...
  for (unsigned int dimCtr = 0; dimCtr < ImageDimension; ++dimCtr)
  {
    ...
    for (int regCtr = 1; regCtr < (numIn[dimCtr] + numPre[dimCtr] + numPost[dimCtr]); ++regCtr)
    {
      if (minIndex[dimCtr] == maxIndex[dimCtr])
      {
        minIndex[dimCtr] = inputRegionStart[dimCtr][regCtr];
        maxIndex[dimCtr] = minIndex[dimCtr] + static_cast<long>(inputRegionSizes[dimCtr][regCtr]);
      }
      else
      {
        ...
        if ((inputRegionStart[dimCtr][regCtr] + static_cast<long>(inputRegionSizes[dimCtr][regCtr])) > maxIndex[dimCtr])
        {
          maxIndex[dimCtr] = inputRegionStart[dimCtr][regCtr] + static_cast<long>(inputRegionSizes[dimCtr][regCtr]);
        }
      }
    }
  }
  ...
  const OutputImageRegionType & outputRegionForThread)
{
  ...
  for (int regCtr = 0; regCtr < numRegions; ++regCtr)
  {
