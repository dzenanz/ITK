...
template <typename TInputImage, typename TMaskImage, typename TOutputImage, typename TKernel, typename THistogram>
void
MaskedMovingHistogramImageFilter<TInputImage, TMaskImage, TOutputImage, TKernel, THistogram>::SetGenerateOutputMask(
  bool generateOutputMask)
{
  if (generateOutputMask != this->m_GenerateOutputMask)
  {
    ...
    if (generateOutputMask)
    {
      ...
      const typename MaskImageType::Pointer maskout = TMaskImage::New();
...
template <typename TInputImage, typename TMaskImage, typename TOutputImage, typename TKernel, typename THistogram>
void
MaskedMovingHistogramImageFilter<TInputImage, TMaskImage, TOutputImage, TKernel, THistogram>::AllocateOutputs()
{
  if (this->m_GenerateOutputMask)
  {
    ...
    const typename TMaskImage::Pointer mask = this->GetOutputMask();
...
template <typename TInputImage, typename TMaskImage, typename TOutputImage, typename TKernel, typename THistogram>
DataObject::Pointer
MaskedMovingHistogramImageFilter<TInputImage, TMaskImage, TOutputImage, TKernel, THistogram>::MakeOutput(
...
MaskedMovingHistogramImageFilter<TInputImage, TMaskImage, TOutputImage, TKernel, THistogram>::GetOutputMask()
  -> MaskImageType *
{
  const typename MaskImageType::Pointer res = dynamic_cast<MaskImageType *>(this->ProcessObject::GetOutput(1));
  return res;
}
  ...
  DynamicThreadedGenerateData(const OutputImageRegionType & outputRegionForThread)
{
  ...
  while (!InLineIt.IsAtEnd())
  {
    ...
    for (InLineIt.GoToBeginOfLine(); !InLineIt.IsAtEndOfLine(); ++InLineIt)
    {
      ...
      pushHistogram(histRef, addedList, removedList, inputRegion, stRegion, inputImage, maskImage, currentIdx);
    }
    ...
    // Now move the histogram
    pushHistogram(
      tmpHist, addedListLine, removedListLine, inputRegion, stRegion, inputImage, maskImage, PrevLineStartHist);
